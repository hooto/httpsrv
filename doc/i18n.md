...
In v2, i18n is an **opt-in module, not loaded by default**. When you need it, attach it to the App with `WithI18n` and pass the template function `i.Funcs()` to the `Renderer`. Locale matching uses the stdlib `golang.org/x/text` (BCP-47).
...
Pass `i.Funcs()` as extra template functions to the `Renderer`:
